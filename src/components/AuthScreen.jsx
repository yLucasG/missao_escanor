...
import { ShieldAlert, Sun, Lock, User } from "lucide-react";
...
export default function AuthScreen({ onLogin, onSignUp, error }) {
  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center p-4">
      ...
      >
        <div className="flex justify-center mb-6 drop-shadow-[0_0_15px_rgba(168,85,247,0.5)]">
          {/* A sun colored in neon */}
          <SunIcon size={80} glow color="#A855F7" />
        </div>

        <div className="text-center mb-8">
          <h1 className="text-3xl font-black tracking-[0.1em] uppercase text-neon-100 drop-shadow-sm">
            Missão <span className="text-neon-500">Scanor</span>
          </h1>
          <p className="text-xs uppercase tracking-[0.2em] text-neon-400 mt-2 font-bold">
            Domínio Neon Purpura
          </p>
        </div>
